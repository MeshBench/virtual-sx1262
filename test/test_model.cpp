...
#define CASE(name) do { g_case = name; std::printf("\n  %s\n", name); } while (0)
...
int main()
{
    ...
    {
        struct Seen { int edges = 0; int level = -1; } seen;
        vsx_chip *c = vsx_create();
        uint64_t now = 0;
        bring_up(c, &now);
        vsx_set_dio1_callback(c, [](void *u, int asserted) {
        ...
        vsx_tick(c, ++now);
        check(seen.edges == 1 && seen.level == 1,
              "delivering the packet asserts DIO1, once");
        ...
        check(seen.edges == 2 && seen.level == 0,
              "acknowledging it drops the line, once");
        vsx_destroy(c);
    }
     ...
     * every repeater in a run believes the channel is busy for ever. */
    {
        ...
        vsx_counters k;
        vsx_get_counters(c, &k);
        check(k.spurious_raises > 0, "the stuck variant raises a flag with nothing on the air");
        vsx_destroy(c);
    ...
    CASE("a transmitted frame comes back out once");
    {
        ...
        const uint8_t pp[] = {0x8C, 0x00, 0x08, 0x00, (uint8_t)sizeof(payload), 0x01, 0x00, 0x00, 0x00};
